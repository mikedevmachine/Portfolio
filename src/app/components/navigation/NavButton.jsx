...
import { Github, Home, Linkedin, Notebook, NotebookText, Palette, Phone, User } from "lucide-react";
...
const NavButton = ({ x, y, label, link, icon, newTab, labelDirection = "right" }) => {
    return (
        ...
        bg-background/20 border border-accent/30 border-solid backdrop-blur-[6px] shadow-glass-inset
         hover:text-white hover:shadow-glass-sm"
                                aria-label={label}
                                name={label}>

                                <span className="relative w-10 h-10 xs:w-14 xs:h-14 p-2.5 xs:p-4 animate-spin-slow-reverse group-hover:pause
                 hover:text-accent">
                                    {getIcon(icon)}
